<script>
	...
	// Start Time
	let start = new Date();
	let a = data.start_time.split(':');
	start.setHours(Number(a[0]));
	start.setMinutes(Number(a[1]));

	// End Time
	let end = new Date();
	let b = data.end_time.split(':');
	end.setHours(Number(b[0]));
	end.setMinutes(Number(b[1]));
